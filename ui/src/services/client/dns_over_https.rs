...
use super::types::{CloudflareDoHResponse, CachedDnsResponse};
...
impl DnsOverHttpsResolver {
    ...
    async fn resolve_txt_single(&self, endpoint: &str, domain: &str) -> Result<Vec<String>, ResolveError> {
        let url = format!("{}?name={}&type=16", endpoint, domain);
        
        info!("Resolving TXT records for {} via {}", domain, endpoint);
        
        let response: CloudflareDoHResponse = self.http_client
            .get(&url)
        ...
        let txt_records: Vec<String> = response.answer
            .unwrap_or_default()
            .into_iter()
            .filter_map(|record| {
                if record.record_type == 16 { // TXT = 16
...
impl DnsResolver for DnsOverHttpsResolver {
    async fn resolve_txt(&self, domain: &str) -> Result<Vec<String>, ResolveError> {
        ...
        match self.resolve_txt_single(&self.primary_endpoint, domain).await {
            Ok(result) => {
        ...
        Err(ResolveError::AllDnsEndpointsFailed { domain: domain.to_string() })
    }
}
...
mod tests {
    ...
    async fn test_doh_resolver_rudyfraser() {
        ...
        let result = resolver.resolve_txt("_atproto.rudyfraser.com").await.unwrap();
    ...
    async fn test_nonexistent_domain() {
        let resolver = DnsOverHttpsResolver::new();
        let result = resolver.resolve_txt("_atproto.nonexistent-domain-12345.com").await;
