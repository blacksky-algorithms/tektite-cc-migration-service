...
use crate::features::migration::{
    steps::plc::setup_plc_transition_client_side, 
...
pub async fn execute_migration_client_side(
        ...
        .await
    {
        ...
        Err(e) => {
            console_error!("{}", format!(
                "[Migration] Failed to describe target PDS: {}",
                e
            ));
    ...
    let service_auth_token =
        match request_service_auth_token(&migration_client, &old_session, &target_pds_did).await {
            ...
            Err(e) => {
                console_error!("{}", format!(
                    "[Migration] Failed to generate service auth token: {}",
                    e
                ));
                dispatch.call(MigrationAction::SetMigrationError(Some(format!(
    ...
    let new_session = match login_result {
        Ok(login_response) => {
            if login_response.success && login_response.session.is_some() {
                ...
                console_info!(
                    "[Migration] Account already exists. Checking migration progress..."
                );
                dispatch.call(MigrationAction::SetMigrationStep(
                ...
                match can_resume_migration(&existing_session).await {
                    Ok(true) => {
                        ...
                        console_info!(
                            "[Migration] Migration can be resumed from existing account"
                        );
                        match get_migration_checkpoint(&existing_session).await {
                            Ok(checkpoint) => {
                                ...
                                match resume_result {
                                    Ok(_) => {
                                        console_info!(
                                            "[Migration] Migration resumed successfully"
                                        );
                                        dispatch.call(MigrationAction::SetMigrating(false));
                                    }
                                    Err(error) => {
                    ...
                    .await
                {
                    ...
                    Err(error) => {
                        console_error!("{}", format!("[Migration] Failed to create account: {}", error.clone()));
        ...
        Err(error) => {
            console_error!("{}", format!(
                "[Migration] Failed to check account existence: {}",
                error
            ));
            dispatch.call(MigrationAction::SetMigrationError(Some(format!(
...
async fn execute_smart_blob_migration(
    ...
    console_info!("[Migration] Step 9: Checking for missing blobs on new PDS");
    dispatch.call(MigrationAction::SetMigrationStep(
...
async fn get_migration_checkpoint(
    ...
    match pds_client.check_account_status(session).await {
        Ok(response) => {
            ...
            } else if response.expected_blobs.unwrap_or(0) > 0
                && response.imported_blobs.unwrap_or(0) > 0
            {
...
/// Verify blob migration completion using simplified count-based comparison
#[cfg(feature = "web")]
async fn verify_and_complete_blob_migration(
    ...
    console_info!("[Migration] Starting simplified blob migration verification...");
    dispatch.call(MigrationAction::SetMigrationStep(
        "Verifying blob migration with count-based comparison...".to_string(),
    ));

    let pds_client = PdsClient::new();
    
    // Get source blob count
    console_info!("[Migration] Getting source PDS blob count...");
    let source_blob_cids = match pds_client.list_all_source_blobs(old_session, &old_session.did).await {
        Ok(cids) => {
            console_info!("{}", format!(
                "[Migration] ✅ Source PDS: {} total blobs found",
                cids.len()
            ));
            cids
        }
        Err(e) => {
            let error_msg = format!("Failed to enumerate source blobs: {}", e);
            console_error!("{}", &error_msg);
            return Err(error_msg);
        }
    };
    let source_count = source_blob_cids.len();

    // Get target blob count
    console_info!("[Migration] Getting target PDS blob count...");
    let target_blob_cids = match pds_client.list_all_target_blobs(new_session, &new_session.did).await {
        Ok(cids) => {
            console_info!("{}", format!(
                "[Migration] ✅ Target PDS: {} total blobs found",
                cids.len()
            ));
            cids
        }
        Err(e) => {
            let error_msg = format!("Failed to enumerate target blobs: {}", e);
            console_error!("{}", &error_msg);
            return Err(error_msg);
        }
    };
    let target_count = target_blob_cids.len();

    // Simple count-based comparison
    console_info!("{}", format!(
        "[Migration] 📊 Count Comparison: Source={} vs Target={}",
        source_count,
        target_count
    ));
    ...
    if source_count == target_count {
        console_info!("{}", format!(
            "[Migration] ✅ Blob migration complete! Source and target have matching counts ({} blobs)",
            source_count
        ));
        return Ok(());
    }
    ...
    if target_count > source_count {
        console_info!("{}", format!(
            "[Migration] ✅ Target has more blobs than source ({} vs {}). Extra blobs preserved, proceeding.",
            target_count,
            source_count
        ));
        return Ok(());
    ...
    // source_count > target_count - attempt reconciliation
    let missing_count = source_count - target_count;
    console_info!("{}", format!(
        "[Migration] 🔄 Target missing {} blobs (source: {}, target: {}), attempting reconciliation...",
        missing_count,
        source_count,
        target_count
    ));
    ...
    dispatch.call(MigrationAction::SetMigrationStep(format!(
        "Uploading {} missing blobs to target PDS...",
        missing_count
    )));

    // Get missing blobs from target PDS API
    let missing_blobs = match pds_client.get_missing_blobs(new_session, None, Some(500)).await {
        Ok(response) => {
            if response.success {
                let blobs = response.missing_blobs.unwrap_or_default();
                console_info!("{}", format!(
                    "[Migration] Found {} missing blobs via target PDS API",
                    blobs.len()
                ));
                blobs
            } else {
                console_warn!("{}", format!(
                    "[Migration] Failed to get missing blobs from target API: {}, continuing anyway",
                    response.message
                ));
                Vec::new()
            }
        ...
        Err(e) => {
            console_warn!("{}", format!(
                "[Migration] Error getting missing blobs from target API: {}, continuing anyway",
                e
            ));
            Vec::new()
        }
    ...
    if !missing_blobs.is_empty() {
        ...
        // Initialize blob manager for migration
        let mut blob_manager = match crate::services::blob::blob_fallback_manager::create_fallback_blob_manager().await {
            Ok(manager) => manager,
            Err(e) => {
                console_warn!("{}", format!(
                    "[Migration] Failed to initialize blob manager: {}, proceeding without reconciliation",
                    e
                ));
                return Ok(()); // Proceed to PLC operations anyway
            }
        };
        ...
        match smart_blob_migration(
            ...
            Ok(result) => {
                console_info!("{}", format!(
                    "[Migration] 📤 Reconciliation completed: {}/{} blobs uploaded, {} failed",
                    result.uploaded_blobs,
                    result.total_blobs,
                    result.failed_blobs.len()
                ));
                ...
                if result.failed_blobs.is_empty() {
                    console_info!("[Migration] ✅ All missing blobs successfully uploaded");
                } else {
                    console_warn!("{}", format!(
                        "[Migration] ⚠️ {} blobs failed to upload, but proceeding to PLC operations",
                        result.failed_blobs.len()
                    ));
                }
            }
            Err(e) => {
                console_warn!("{}", format!(
                    "[Migration] Reconciliation attempt failed: {}, but proceeding to PLC operations",
                    e
                ));
            }
        }
    ...
    // Always proceed to PLC operations regardless of reconciliation outcome
    console_info!("[Migration] ✅ Blob migration verification completed, proceeding to PLC operations");
    Ok(())
}
