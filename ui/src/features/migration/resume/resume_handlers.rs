...
pub async fn can_resume_migration(session: &ClientSessionCredentials) -> Result<bool, String> {
    ...
    match session_manager.get_migration_progress(&session.did) {
        Ok(Some(progress)) => {
            ...
            console::info!("Migration resume check for {}: resumable={}, checkpoint={:?}", 
                session.did.clone(), is_resumable, progress.last_checkpoint);
        ...
        Err(e) => {
            console::warn!(&format!("Failed to check migration progress: {}", e));
...
pub fn get_resume_checkpoint(progress: &crate::features::migration::types::MigrationProgress) -> Option<crate::features::migration::types::MigrationCheckpoint> {
...
pub async fn resume_migration_from_checkpoint(
    ...
    let progress = session_manager.get_migration_progress(&old_session.did)
    ...
    console::info!("Resuming migration from checkpoint: {}", checkpoint_name);
    ...
    match checkpoint {
        ...
        MigrationCheckpoint::PlcReady => {
            console::info!("Migration is at PLC ready checkpoint - manual form submission required");
...
pub async fn resume_from_repo_migration(
    ...
    console::info!("[Resume] Resuming from repository migration step");
...
pub async fn resume_from_blob_migration(
    ...
    console::info!("[Resume] Resuming from blob migration step");
...
pub async fn resume_from_preferences_migration(
    ...
    console::info!("[Resume] Resuming from preferences migration step");
...
pub async fn resume_from_plc_operations(
    ...
    console::info!("[Resume] Resuming from PLC operations step");
