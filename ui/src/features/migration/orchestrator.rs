...
use crate::features::migration::{
    resume::{
        can_resume_migration, resume_from_blob_migration, resume_from_plc_operations,
        resume_from_preferences_migration, resume_from_repo_migration,
    },
    steps::{
...
pub async fn execute_migration_client_side(
    ...
    match can_resume_migration(&new_session).await {
        Ok(true) => {
            console_info!("[Migration] Previous migration detected - checking resumption point");
            if let Err(e) =
                attempt_resume_migration(&state, &dispatch, &old_session, &new_session).await
            {
                console_warn!(
                    "[Migration] Resume failed: {} - starting fresh migration",
                    e
                );
            } else {
                console_info!("[Migration] Successfully resumed previous migration");
                return;
            }
        }
        Ok(false) => {
            console_info!("[Migration] No previous migration found - starting fresh");
        }
        Err(e) => {
            console_warn!(
                "[Migration] Failed to check resume capability: {} - starting fresh",
                e
            );
        }
    }
...
async fn execute_full_migration(
    ...
    console_info!("[Migration] Phase 4: PLC Transition Setup");
...
async fn is_blobs_migrated(session: &ClientSessionCredentials) -> bool {
    ...
    match client.get_missing_blobs(session, None, None).await {
        Ok(response) => response.success && response.missing_blobs.unwrap_or_default().is_empty(),
        Err(_) => false,
    }
}
